...
const char* data_file_addr = "../input/uscounties.csv";
    ...
    public:
        ...
        void search_nearest(string method, float la, float lo, int k, County* k_nearest_counties)
        {
            ...
            if(method.compare("knn") == 0)
            {
                ...
                // period_time = (double)(endt - startt);
                // cout << "Search Time Cost: " << period_time << " ms" << endl;
                cout << "Search Time Cost: " << diff.count() * 1000 << " ms" << endl;

                for(int i = 0; i < k; i++)
                {
                    cout<< i << ": county: " << k_nearest_counties[i].name << " state: " << k_nearest_counties[i].state << endl;
                }
            }
